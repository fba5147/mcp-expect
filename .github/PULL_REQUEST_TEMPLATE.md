## What does this change and why?

## Test coverage

- [ ] Added or extended a test in `example/*.mcptest.ts` against a real server
- [ ] `npm run build` passes (`tsc --strict`)
- [ ] Ran the affected suite(s) locally and confirmed pass/fail output is correct

## Docs

- [ ] Updated the README if this changes public API, scope, or a documented claim
      (performance numbers, dependency list, etc.) — verified against real code/measurements,
      not just written from memory
